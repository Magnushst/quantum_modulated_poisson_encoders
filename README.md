...
round-trips, without attributing an accuracy benefit to the tested PQC.
...
* A controlled drive ablation (PQC vs. parameter-count-controlled classical head vs.
...
| `expressivity_probe.py` | Controlled ablation: input-conditioned PQC vs. parameter-count-controlled classical head vs. constant, depths 1/2/4 | `expressivity.csv`, `expressivity_verdict.txt` |
...
$B=256$ comes from a separate paging-free isolated forward loop. **Estimated**
(first-principles; labelled as such in the paper):
